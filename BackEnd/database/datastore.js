...
export class Data_App {
  ...
  static Users = {
    async Delete (userId){
      for await (const [index, element] of Data_App.users.entries()) {
        if (element.id === userId) {
            if (index != -1) Data_App.users.splice(index, 1);
            await sql(`DELETE FROM Users WHERE id="${userId}"`)
          }
          
    }},
    async SaveChange(userId,data){
      for (const [index, element] of Data_App.users.entries()) {
        if (element.id === userId) {
           (Data_App.users[index]["password"] = data.password);
           (Data_App.users[index]["acl"] = data.acl);
        }
      }
      await sql(`update Users SET password="${data.password}",acl="${data.acl}" where id="${userId}"`)
    },
    ...
    async create(data) {
      ...
      for await (const [index, user] of userRows.entries()) {
        userRows[index].name = await JSON.parse(user.name);
        Data_App.users.push(userRows[index])
      }
    },
