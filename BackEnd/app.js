...
const io = new Server(server, {
  ...
  cors: {
    origin: ["https://192.168.8.76", "https://192.168.8.76:3002"],
...
io.on("connection", async (socket) => {
  ...
  socket.on("GetUser_admin_detail", async (userid) => {
    try {
      let data = await Data_App.Users.GetUserData(parseInt(userid));
      io.to(socket.id).emit("GetUser_admin_detail_", { password: data.password, acl: data.acl });
    } catch (error) {
      console.log(error);
    }
  });
  socket.on("SaveUser_admin_detail", async (userid, data) => {
    try {
      await Data_App.Users.SaveChange(parseInt(userid), data);
      io.to(socket.id).emit("GotoBack___");
    } catch (error) {
      console.log(error);
    }
  });
  socket.on("DeleteUser_admin_detail", async (userid) => {
    try {
      await Data_App.Users.Delete(parseInt(userid));
      io.to(socket.id).emit("GotoBack___");
    } catch (error) {
      console.log(error);
    }
